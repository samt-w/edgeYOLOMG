...
def parse_opt(known=False):
    ...
    # removing the default of importing pretrained YOLOv5s weights due to architecture incompatibility
...
if __name__ == "__main__":
    ...
    # extract data if it has not previously been extracted
    data_dir = ROOT / "data_processed_ARD100"
    ...
    # terminate the pod automatically at the end of the training run
    load_dotenv()
    pod_id = os.getenv("RUNPOD_POD_ID")
    api_key = os.getenv("RUNPOD_API_KEY")

    if pod_id and api_key:
        print(f"Training complete. Terminating pod {pod_id}...")
        runpod.api_key = api_key
        runpod.terminate_pod(pod_id)
    else:
        print("Could not terminate pod: RUNPOD_POD_ID or API_KEY missing.")