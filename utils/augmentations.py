...
class Albumentations:
    ...
    def __call__(self, im,im2, labels, p=1.0):
        if self.transform and random.random() < p:
            new = self.transform(image=im, bboxes=labels[:, 1:], class_labels=labels[:, 0])  # transformed
            new2 = self.transform(image=im2, bboxes=labels[:, 1:], class_labels=labels[:, 0])  # transformed
            im, labels = new['image'], np.array([[c, *b] for c, b in zip(new['class_labels'], new['bboxes'])])
            im2, labels = new2['image'], np.array([[c, *b] for c, b in zip(new2['class_labels'], new2['bboxes'])])
        return im,im2, labels


def augment_hsv(im, hgain=0.8, sgain=0.9, vgain=0.8):
...
def copy_paste(im,im2, labels, segments, p=0.5):
    ...
    if p and n:
        ...
        for j in random.sample(range(n), k=round(p * n)):
            ...
            if (ioa < 0.30).all():  # allow 30% obscuration of existing labels
                ...
                cv2.drawContours(im_new2, [segments[j].astype(np.int32)], -1, (255, 255, 255), cv2.FILLED)

        result = cv2.bitwise_and(src1=im, src2=im_new)
        result2 = cv2.bitwise_and(src1=im2, src2=im_new2)
        result = cv2.flip(result, 1)  # augment segments (flip left-right)
        result2 = cv2.flip(result2, 1)  # augment segments (flip left-right)
        i = result > 0  # pixels to replace
        # i[:, :] = result.max(2).reshape(h, w, 1)  # act over ch
        im[i] = result[i]  # cv2.imwrite('debug.jpg', im)  # debug
        im2[i] = result2[i]  # cv2.imwrite('debug.jpg', im)  # debug
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=100, area_thr=0.1, eps=1e-16):  # box1(4,n), box2(4,n)
    ...
    return (w2 > wh_thr) & (h2 > wh_thr) & (w2 * h2 / (w1 * h1 + eps) > area_thr) & (ar < ar_thr)  # candidates 宽度高度大于2，面积要大于阈值
