"""
this script is adapted from inference_jetson.py - it simulates deploying the optimised YOLOMG
TensorRT .engine on Jetson Orin Nano (e.g. with realistic confidence threshold, and not 
measuring accuracy)
"""
...
def process_predictions(pred, h0, w0, processed_shape):
    """
    Replaces evaluate_frame() for deployment - just returning predictions, not computing
    IoU (because there is no labelled dataset)

    Scales coordinates back to original dimensions and formats for onwards transmission to 
    a user
    """
    if len(pred) == 0:
        return None
        
    # rescale predictions from the padded image to native original image dimensions
    predn = pred.clone()
    predn[:, :4] = scale_coords(processed_shape, predn[:, :4], (h0, w0)).round()
    
    # return the final scaled bounding boxes, confidences, and classes
    return predn.cpu().numpy()
...
def calculate_metrics(timings,
    ...
    if run_config["video_name"] == "OVERALL_SUMMARY":
        master_csv_file = output_dir.parent / "YOLOMG-Deployment-results.csv"
...
def run_inference_directory(video_dir,
    ...
    for video_path in video_files:
        ...
        calculate_metrics(video_timings, video_inference_count, run_config)
    ...
    if overall_inference_count > 0:
        ...
        calculate_metrics(overall_timings, overall_inference_count, summary_config)
