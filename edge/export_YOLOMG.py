        ...
        if simplify:
            try:
                ...
                model_onnx, check = onnxsim.simplify(
                    ...
                    input_shapes={'images': list(im[0].shape), 'masks': list(im[1].shape)} if dynamic else None)
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    im_rgb = torch.zeros(batch_size, 3, *imgsz).to(device)
    im_mask = torch.zeros(batch_size, 3, *imgsz).to(device) 
    
    im = (im_rgb, im_mask) # Group as a tuple for ONNX

    # Update model
    if half:
        im = (im[0].half(), im[1].half())
    ...
    for _ in range(2):
        y = model(*im)  # dry runs; unpacks tuple to x1, x2
